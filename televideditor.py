...
def download_telegram_file(file_id, job_id, media_type=None):
    """Downloads a file from Telegram using a file_id or directly from a web URL."""
    try:
        if file_id.startswith("http://") or file_id.startswith("https://"):
            file_extension = ".mp4" if media_type == 'video' else ".jpg"
            url_path = file_id.split('?')[0]
            ...
            logging.info(f"Successfully downloaded web media to {save_path}")
            return save_path
...
def process_video_job(job_data):
    ...
    try:
        media_path = download_telegram_file(job_data['file_id'], job_id, job_data.get('media_type'))
